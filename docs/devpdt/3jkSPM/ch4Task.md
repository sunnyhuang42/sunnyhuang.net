...
- 现行的需求理解策略已基本能满足大多数用户的需求。原因如下：
  ...
  - 所以，即使现行需求理解策略离 「Google 这样 search engine 标杆」 相差甚远，也已够用。如有更优先的需求，现行需求理解策略暂不优化也无大碍。
...
但这两天测试发现，除了支持与、或、非 3 种常见语法搜索外，其它更高阶的搜索功能基本都已失效。暂未检索到此功能下线原因，介于其上线时亦低调，推测是图谱功能若要进一步迭代成本太高，而国内亦有大众点评、豆瓣等主流平台满足上述提及的图谱需求，留着也鸡肋，所以低调下线。
